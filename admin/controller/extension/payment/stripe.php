...
class ControllerExtensionPaymentStripe extends Controller { 
	...
	public function index() {
		...
		$data['settings'][] = array(
			'key'		=> 'use_checkout',
			'type'		=> 'select',
			'options'	=> array('all' => $data['text_yes'], 'desktop' => $data['text_yes_for_desktop_devices'], 0 => $data['text_no']),
			'default'	=> 0,
		);
		$data['settings'][] = array(
	...
	public function capture() {
		...
		$capture_response = $this->curlRequest('POST', 'charges/' . $this->request->get['charge_id'] . '/capture', array('amount' => $this->request->get['amount'] * 100));
		...
		if (empty($capture_response['error']) || strpos($capture_response['error']['message'], 'has already been captured')) {
			$this->db->query("UPDATE " . DB_PREFIX . "order_history SET `comment` = REPLACE(`comment`, '<span>No &nbsp;</span> <a', 'Yes (" . number_format($this->request->get['amount'], 2, '.', '') . " captured) <a style=\"display: none\"') WHERE `comment` LIKE '%capture($(this), %, \'" . $this->db->escape($this->request->get['charge_id']) . "\')%'");
		}
	}
