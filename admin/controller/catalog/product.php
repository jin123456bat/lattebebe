...
// 		exit();
	...
	protected function getList() {
		...
		$data['import'] = $this->url->link('catalog/product/import&user_token=' . $this->session->data['user_token'] . $url,'SSL');
	...
	public function import(){
		...
		if (true)
		{
			//if (isset($_FILES['import']) && $_FILES['import']['error'] == 0 && $_FILES['import']['size']>0 && $_FILES['import']['type'] == 'text/xml')
			if (true)
			{
				//if (file_exists($_FILES['import']['tmp_name']) && is_readable($_FILES['import']['tmp_name']))
				if (true)
				{
					//$content = file_get_contents($_FILES['import']['tmp_name']);
					$content = file_get_contents('/var/www/opencart/prodotti.xml');
					libxml_disable_entity_loader(true);
					$content = simplexml_load_string($content, 'SimpleXMLElement', LIBXML_NOCDATA);
					//$content = simplexml_load_file($_FILES['import']['tmp_name'],'SimpleXMLElement',LIBXML_NOCDATA);
					...
					foreach ($content['prodotto'] as $product)
					{
// 						var_dump($product);exit();
// 						//原价
// 						$price = 0;
// 						if (empty($data['price']) && !is_array($product['p_bd']) && !empty($product['p_bd']))
// 						{
// 							$price = $product['p_bd'];
// 						}
						
// 						if (empty($data['price']) && !is_array($product['p_ven']) && !empty($product['p_ven']))
// 						{
// 							$price = $product['p_ven'];
// 						}
						
						$this->model_catalog_product->updateModel($product['minsan'],$product['ean']);
						
						//$this->model_catalog_product->insertProductSpecial($price,$product['minsan'],$product['p_ven']);
					}
...
// 						$image = '';
// 						if (is_string($product['url_img']) && !empty($product['url_img']))
// 						{
// 							$filename = md5($product['url_img']).'.'.pathinfo($product['url_img'],PATHINFO_EXTENSION);
// 							$image = 'catalog/product/'.$filename;
// 							if (!file_exists(DIR_IMAGE.$image))
// 							{
// 								$image_content = file_get_contents($product['url_img']);
// 								file_put_contents(DIR_IMAGE.$image, $image_content);
// 							}
// 						}
						
// 						$quantity = 0;
// 						if (!is_array($product['giac']))
// 						{
// 							$quantity = $product['giac'];
// 						}
						
// 						$price = 0;
// 						if (!is_array($product['p_ven']))
// 						{
// 							$price = $product['p_ven'];
// 						}
// 						if (empty($price) && !is_array($product['p_bd']))
// 						{
// 							$price = $product['p_bd'];
// 						}
						
// 						$data = array(
// 							'model' => $product['minsan'],
// 							'sku' => $product['minsan'],
// 							'upc' => $product['minsan'],
// 							'ean' => $product['ean'],
// 							'jan'=>'',
...
// 							'stock_status_id' => '',
// 							'image' => $image,
// 							'manufacturer_id' => intval($product['ditta']),
// 							'shipping' => 1,
// 							'price' => $price,
// 							'points' => '',
// 							'tax_class_id' => '',
...
// 								2 => array(
// 									'name' => (string)$product['descri'],
// 									'description' => (string)$product['info_prof'],
// 									'tag' => '',
// 									'meta_title' => (string)$product['descri'],
// 									'meta_description' => '',
// 									'meta_keyword' => '',
// 								),
// 								3 => array(
...
// 								)
// 							),
// 							'product_category' => $this->model_catalog_category->getCategoryIdByCodice($product['atcgmp']),
// 						);
...
// 					}
					...
					exit("导入完成");
					//上传完毕
					//$this->response->redirect($this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'], 'SSL'));
					//exit();
				}
		...
		else
		{
			...
			$data['action'] = $this->url->link('catalog/product/import','&user_token=' . $this->session->data['user_token'],'SSL');
			$data['cancel'] = $this->url->link('catalog/product','&user_token=' . $this->session->data['user_token'],'SSL');
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] , 'SSL')
			);
			
			$this->response->setOutput($this->load->view('catalog/product_import', $data));
		}
	}
