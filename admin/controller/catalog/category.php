...
class ControllerCatalogCategory extends Controller {
	...
	protected function getList() {
		...
		$data['import'] = $this->url->link('catalog/category/import&user_token=' . $this->session->data['user_token'] . $url,'SSL');
	...
	public function import(){
		if (strtolower($_SERVER['REQUEST_METHOD']) == 'post')
		{
			if (isset($_FILES['import']) && $_FILES['import']['error'] == 0 && $_FILES['import']['size']>0 && $_FILES['import']['type'] == 'text/xml')
			{
				if (file_exists($_FILES['import']['tmp_name']) && is_readable($_FILES['import']['tmp_name']))
				{
					...
					$root = [];
					foreach ($content['record'] as $category)
					{
						if (strlen($category['codice']) == 1)
						{
							$root[$category['codice']] = [
								'name' => $category['descrizione'],
								'sub' => [],
							];
						}
						else if (strlen($category['codice']) == 3)
						{
							$a = substr($category['codice'], 0,1);
							$b = substr($category['codice'], 1,2);
						...
						else if (strlen($category['codice']) == 4)
						{
							$a = substr($category['codice'], 0,1);
							$b = substr($category['codice'], 1,2);
							$c = substr($category['codice'], 3,1);
						...
						else if (strlen($category['codice']) == 5)
						{
							$a = substr($category['codice'], 0,1);
							$b = substr($category['codice'], 1,2);
							$c = substr($category['codice'], 3,1);
							$d = substr($category['codice'], 4,1);
							
							$root[$a]['sub'][$b]['sub'][$c]['sub'][$d] = [
								'name' => $category['descrizione'],
								'sub' => [],
							];
						}
						else if (strlen($category['codice']) == 7)
						{
							$a = substr($category['codice'], 0,1);
							$b = substr($category['codice'], 1,2);
							$c = substr($category['codice'], 3,1);
							$d = substr($category['codice'], 4,1);
							$e = substr($category['codice'], 5,2);
							
							
							$root[$a]['sub'][$b]['sub'][$c]['sub'][$d]['sub'][$e] = [
								'name' => $category['descrizione'],
								'sub' => [],
							];
						}
					...
					foreach ($root as $a => $v)
					{
						...
						$aid = $this->model_catalog_category->addCategory($data);
						foreach ($v['sub'] as $b => $vv)
						{
							$data = [
								'sort_order' => 0,
								'status' => 1,
							...
							$bid = $this->model_catalog_category->addCategory($data);
							foreach ($vv['sub'] as $c => $vvv)
							{
								$data = [
									'sort_order' => 0,
									'status' => 1,
								...
								$cid = $this->model_catalog_category->addCategory($data);
								foreach ($vvv['sub'] as $d => $vvvv)
								{
									$data = [
										'sort_order' => 0,
										'status' => 1,
									...
									$did = $this->model_catalog_category->addCategory($data);
									foreach ($vvvv['sub'] as $e => $vvvvv)
									{
										$data = [
											'sort_order' => 0,
											'status' => 1,
					...
					//上传完毕
					$this->response->redirect($this->url->link('catalog/category', 'user_token=' . $this->session->data['user_token'], 'SSL'));
					exit();
				}
			}
		}
		else
		{
			...
			$data['action'] = $this->url->link('catalog/category/import','&user_token=' . $this->session->data['user_token'],'SSL');
			$data['cancel'] = $this->url->link('catalog/category','&user_token=' . $this->session->data['user_token'],'SSL');
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('catalog/category', 'user_token=' . $this->session->data['user_token'] , 'SSL')
			);
			
			$this->response->setOutput($this->load->view('catalog/category_import', $data));
		}
	}
