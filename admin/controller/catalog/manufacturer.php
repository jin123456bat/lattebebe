...
class ControllerCatalogManufacturer extends Controller {
	...
	public function import(){
		if (strtolower($_SERVER['REQUEST_METHOD']) == 'post')
		{
			if (isset($_FILES['import']) && $_FILES['import']['error'] == 0 && $_FILES['import']['size']>0 && $_FILES['import']['type'] == 'text/xml')
			{
				if (file_exists($_FILES['import']['tmp_name']) && is_readable($_FILES['import']['tmp_name']))
				{
					...
					foreach ($content['record'] as $manufacturer)
					{
						$data = array(
							'id' => $manufacturer['codice'],
							'name' => $manufacturer['descrizione'],
							'sort_order'=>0,
						);
					...
					//上传完毕
					$this->response->redirect($this->url->link('catalog/manufacturer', 'user_token=' . $this->session->data['user_token'], 'SSL'));
					exit();
				}
			}
		}
		else
		{
			...
			$data['action'] = $this->url->link('catalog/manufacturer/import','&user_token=' . $this->session->data['user_token'],'SSL');
			$data['cancel'] = $this->url->link('catalog/manufacturer','&user_token=' . $this->session->data['user_token'],'SSL');
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('catalog/manufacturer', 'user_token=' . $this->session->data['user_token'] , 'SSL')
			);
			
			$this->response->setOutput($this->load->view('catalog/manufacturer_import', $data));
		}
	}
	...
	protected function getList() {
		...
		$data['import'] = $this->url->link('catalog/manufacturer/import&user_token=' . $this->session->data['user_token'] . $url,'SSL');
