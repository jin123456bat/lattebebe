...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), date_added = NOW(),codice='".(string)$data['codice']."'");
	...
	public function getCategoryIdByCodice($codice)
	{
		$t = [];
		$result = $this->db->query('select category_id from '.DB_PREFIX.'category where codice="'.$codice.'" limit 1');
