...
class ModelCatalogProduct extends Model {
	...
	public function updateModel($model,$newmodel)
	{
		if (!empty($model) && !empty($newmodel))
		{
			$this->db->query('update '.DB_PREFIX.'product set ean="'.$newmodel.'" where model="'.$model.'" limit 1');
		}
	}
	
	public function insertProductSpecial($oldprice,$model,$price)
	{
		$oldprice = str_replace(',', '.', $oldprice);
		$price = str_replace(',', '.', $price);
		
		if (!empty($oldprice) && !empty($model))
		{
			$sql = 'update '.DB_PREFIX.'product set price='.$oldprice.' where model="'.$model.'" limit 1';
			$this->db->query($sql);
		}
		
		if (!empty($price) && !empty($model))
		{
			$sql = 'select product_id from '.DB_PREFIX.'product where model="'.$model.'" limit 1';
			$result = $this->db->query($sql);
			$product_id = $result->row['product_id'];
			
			$sql = 'insert into '.DB_PREFIX.'product_special values(NULL,'.$product_id.',1,0,'.$price.',"2000-01-01","2030-12-32")';
			$this->db->query($sql);
		}
	}
	...
	{
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product where sku='".$sku."' limit 1");
		return $query->row;
	}
