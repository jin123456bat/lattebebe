...
class ModelCatalogManufacturer extends Model {
	public function addManufacturer($data) {
		...
		{
			$count = $this->db->query('select * from '.DB_PREFIX.'manufacturer where manufacturer_id='.(int)$data['id'].' limit 1');
			if($count->num_rows == 0)
			{
				$this->db->query("INSERT INTO " . DB_PREFIX . "manufacturer SET manufacturer_id=".((int)$data['id']).", name = '" . $this->db->escape((string)$data['name']) . "', sort_order = '" . (int)$data['sort_order'] . "'");
			}
		}
