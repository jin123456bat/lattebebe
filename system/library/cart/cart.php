...
class Cart
{
	...
	{
		$sql = 'select product_id from '.DB_PREFIX.'cart where cart_id='.$cart_id.' limit 1';
		...
		if (intval($result->row['quantity']) < intval($quantity))
		{
	...
	{
		$sql = "SELECT sum(quantity) as num FROM " . DB_PREFIX . "cart WHERE product_id=" . $product_id . " and api_id = '" . (isset($this->session->data['api_id']) ? (int) $this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int) $this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "'";
