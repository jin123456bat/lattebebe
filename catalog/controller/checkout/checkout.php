...
class ControllerCheckoutCheckout extends Controller {
	private $ADDRESS_FIELDS = array('fullname', 'telephone', 'company', 'address_1', 'address_2', 'city', 'postcode', 'country_id','cardno', 'zone_id','zone','county', 'city_id', 'county_id', 'custom_field','taxno');
	...
	public function __construct($registry) {
		...
		$this->load->model('account/sender_address');
	...
	public function index() {
		...
		$data['sender_address_section'] = $this->renderSenderAddressSection();
	...
	public function zipcode()
	{
		...
		$zipcode = $this->model_account_address->getzipcode($id);
		echo $zipcode;
		exit();
	}
	...
	public function confirm() {
		...
		$order_data['sender_address'] = array();

		// sender address
		if ($this->getPostValue('sender_address') == 'new')
		{
			if ($_error = $this->validateSenderAddress()) {
				$error['sender_address'] = $_error;
			} else {
				foreach ($this->ADDRESS_FIELDS as $field) {
					$order_data['sender_address'][$field] = $this->getPostValue('sender_address_' . $field);
				}
				
				$sender_address_id = $this->model_account_sender_address->addAddress($this->customer->getId(),$order_data['sender_address']);
				
				$this->syncAddressSession('sender', $order_data['sender_address']);
			}
		}
		else if($this->getPostValue('sender_address') == 'existing')
		{
			$address_info = $this->model_account_sender_address->getAddress($this->getPostValue(('sender_address_id')));
			if (!$address_info) {
			...
			$order_data['sender_address'] = $address_info;
			$this->syncAddressSession('sender', $address_info);
		}
		else
		{
			$order_data['sender_address'] = [
				'zone' => $this->config->get('config_sender_default_zone'),
				'city' => $this->config->get('config_sender_default_city'),
				'county' => $this->config->get('config_sender_default_county'),
				'fullname' => $this->config->get('config_sender_default_fullname'),
				'telephone' => $this->config->get('config_sender_default_telephone'),
				'address' => $this->config->get('config_sender_default_address'),
			];
	...
	// Views
	private function renderSenderAddressSection() {
		$this->log(__FUNCTION__);
		$data['logged'] = $this->isLogged();
		
		if (isset($this->session->data['sender_address']['address_id'])) {
			$data['sender_address_id'] = $this->session->data['sender_address']['address_id'];
		} else {
			$data['sender_address_id'] = $this->customer->getSenderAddressId();
		}
		
		$this->load->model('account/sender_address');
		
		$data['addresses'] = $this->model_account_sender_address->getAddresses();
		
		if (isset($this->session->data['sender_address']['postcode'])) {
			$data['postcode'] = $this->session->data['sender_address']['postcode'];
		} else {
			$data['postcode'] = '';
		}
		...
		if (isset($this->session->data['sender_address']['zone_id'])) {
			$data['zone_id'] = $this->session->data['sender_address']['zone_id'];
		} else {
			$data['zone_id'] = '';
		}
		...
		if (isset($this->session->data['sender_address']['custom_field'])) {
			$data['shipping_address_custom_field'] = $this->session->data['shipping_address']['custom_field'];
		} else {
			$data['shipping_address_custom_field'] = array();
		}
		
		return $this->load->view('checkout/checkout/_sender_address', $data);
	}
	...
	private function syncAddressSession($type, $address_info) {
		if (!in_array($type, ['payment', 'shipping','sender'])) {
			return false;
		}
	...
	private function validateSenderAddress() {
		$error = array();
		
		if ((utf8_strlen(trim($this->request->post['sender_address_fullname'])) < 1) || (utf8_strlen(trim($this->request->post['sender_address_fullname'])) > 32)) {
			$error['fullname'] = $this->language->get('error_fullname');
		}
		
		if ((utf8_strlen(trim($this->request->post['sender_address_telephone'])) < 1) || (utf8_strlen(trim($this->request->post['sender_address_telephone'])) > 32)) {
			$error['telephone'] = $this->language->get('error_telephone');
		}
		
		if ((utf8_strlen(trim($this->request->post['sender_address_address_1'])) < 3) || (utf8_strlen(trim($this->request->post['sender_address_address_1'])) > 128)) {
			$error['address_1'] = $this->language->get('error_address_1');
		}
		
		if (empty($this->request->post['sender_address_zone']))
		{
			$error['zone'] = $this->language->get('error_input_zone');
		}
		
		if (empty($this->request->post['sender_address_city']))
		{
			$error['city'] = $this->language->get('error_input_city');
		}
		
		if (empty($this->request->post['sender_address_county']))
		{
			$error['county'] = $this->language->get('error_input_county');
		}
		
// 		$this->load->model('localisation/country');
// 		$countries = $this->model_localisation_country->getCountries();
// 		if (count($countries) > 1) {
// 			if ($this->request->post['sender_address_country_id'] == '') {
// 				$error['country'] = $this->language->get('error_country');
// 				$this->request->post['sender_address_country_id'] = 0;
// 			}
// 		} else {
// 			$this->request->post['sender_address_country_id'] = $this->model_checkout_checkout->getDefaultCountryId();
// 		}
		
		$country_info = $this->model_localisation_country->getCountry($this->request->post['sender_address_country_id']);
		if ($country_info && $country_info['postcode_required'] && (utf8_strlen(trim($this->request->post['sender_address_postcode'])) < 2 || utf8_strlen(trim($this->request->post['sender_address_postcode'])) > 10)) {
			$error['postcode'] = $this->language->get('error_postcode');
		}
		
// 		if (!isset($this->request->post['sender_address_zone_id']) || $this->request->post['sender_address_zone_id'] == '' || !is_numeric($this->request->post['sender_address_zone_id'])) {
// 			$error['zone'] = $this->language->get('error_zone');
// 		}
		
// 		if (!isset($this->request->post['sender_address_city_id']) || (int)$this->request->post['sender_address_city_id'] <= 0) {
// 			$error['city'] = $this->language->get('error_city');
// 		}
		
// 		if (!isset($this->request->post['sender_address_county_id']) || (int)$this->request->post['sender_address_county_id'] <= 0) {
...
// 		}
		...
		foreach ($custom_fields as $custom_field) {
			if (($custom_field['location'] == 'address') && $custom_field['required'] && empty($this->request->post['sender_address_custom_field'][$custom_field['custom_field_id']])) {
				$error['custom_field' . $custom_field['custom_field_id']] = sprintf($this->language->get('error_custom_field'), $custom_field['name']);
			} elseif (($custom_field['location'] == 'address') && ($custom_field['type'] == 'text') && !empty($custom_field['validation']) && !filter_var($this->request->post['sender_address_custom_field'][$custom_field['custom_field_id']], FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => $custom_field['validation'])))) {
				$error['custom_field' . $custom_field['custom_field_id']] = sprintf($this->language->get('error_custom_field'), $custom_field['name']);
			}
		}
	...
	private function validateShippingAddress() {
		...
		//验证身份证号码
		if (!isset($this->request->post['shipping_cardno']) || empty($this->request->post['shipping_cardno']) || strlen($this->request->post['shipping_cardno'])!=18)
		{
			$error['cardno'] = $this->language->get('error_cardno');
		}
