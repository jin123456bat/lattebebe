...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			include_once DIR_STORAGE.'phpqrcode/phpqrcode.php';
			$object = new \QRcode();
			$rootPath = sys_get_temp_dir();
			$path = tempnam($rootPath, 'qrcode');
			$errorCorrectionLevel = 'L';    //容错级别
			$matrixPointSize = 5;
			$url = htmlspecialchars_decode($this->url->link('product/product', $url . '&product_id=' . $this->request->get['product_id']));
			$object->png($url,$path,$errorCorrectionLevel, $matrixPointSize, 2);
			$data['qrcode'] = 'data:image/png;base64,' . base64_encode(file_get_contents($path));
			
			$data['barcode'] = $this->url->link('product/product/barcode', 'text='.$data['ean']);
	...
	{
		require_once(DIR_STORAGE."barcode/class/BCGFontFile.php");
		require_once(DIR_STORAGE."barcode/class/BCGColor.php");
		require_once(DIR_STORAGE."barcode/class/BCGDrawing.php");
		/* 根据不同的编码方法，引入相应的库文件 */
		require_once(DIR_STORAGE."barcode/class/BCGcode39.barcode.php");
		/* 颜色和字体 */
		$colorFront = new BCGColor(0,0,0);
		$colorBack = new BCGColor(255,255,255);
		$font = new BCGFontFile(DIR_STORAGE."barcode/font/Arial.ttf", 12);
		/* 编码对象 */
		$code = new BCGcode39();
		$code->setScale(2);
