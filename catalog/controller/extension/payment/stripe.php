...
class ControllerExtensionPaymentStripe extends Controller {
	...
	public function chargeSource() {
			...
			foreach ($plans as &$plan) {
				...
				if (!empty($customer_response['subscriptions'])) {
				 	foreach ($customer_response['subscriptions']['data'] as $subscription) {
						if ($subscription['plan']['id'] == $plan['id']) $current_quantity += $subscription['quantity'];
					}
				}
				...
				$subscription_data = array(
					...
					'items'			=> array(array('plan' => $plan['id'], 'quantity' => $current_quantity + $plan['quantity'])),
				...
				if ($subscription_response['status'] == 'trialing') {
		...
		if (!empty($charge_response)) {
			...
			$comment .= $strong . 'Captured:</strong>' . (!empty($charge_response['captured']) ? 'Yes' : '<span>No &nbsp;</span> <a onclick="stripeCapture($(this), ' . number_format($charge_amount, 2, '.', '') . ', \'' . $charge_response['id'] . '\')">(Capture)</a>') . '<br>';
			...
			// Card fields (source)
			$card = (!empty($charge_response['source']['card'])) ? $charge_response['source']['card'] : '';
	...
	public function webhook() {
		...
		if ($event->type == 'charge.captured') {
			...
			*/
			...
			if ($shipping) {
				$address_query = $this->db->query("SELECT address_id FROM " . DB_PREFIX . "customer WHERE customer_id = " . (int)$data['customer_id']);
				if ($address_query->num_rows) {
					$default_address = $this->db->query("SELECT * FROM " . DB_PREFIX . "address WHERE address_id = " . (int)$address_query->row['address_id'])->row;
					$default_address['zone'] = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone WHERE zone_id = " . (int)$default_address['zone_id'])->row['name'];
					$default_address['country'] = $this->db->query("SELECT * FROM " . DB_PREFIX . "country WHERE country_id = " . (int)$default_address['country_id'])->row['name'];
					
					$data['shipping_firstname'] = $default_address['firstname'];
					$data['shipping_lastname'] = $default_address['lastname'];
					$data['shipping_company'] = $default_address['company'];
					$data['shipping_company_id'] = '';
					$data['shipping_tax_id'] = '';
					$data['shipping_address_1'] = $default_address['address_1'];
					$data['shipping_address_2'] = $default_address['address_2'];
					$data['shipping_city'] = $default_address['city'];
					$data['shipping_postcode'] = $default_address['postcode'];
					$data['shipping_zone'] = $default_address['zone'];
					$data['shipping_zone_id'] = $default_address['zone_id'];
					$data['shipping_country'] = $default_address['country'];
					$data['shipping_country_id'] = $default_address['country_id'];
				} else {
