...
class ModelCheckoutOrder extends Model
{
	...
	public function addOrder($data)
	{
		...
		if ($data['shipping_country_id'] == 44)
		{
			$total_num = 0;
			$sku_num = 0;
			$total_money  = 0;
			...
			if (isset($data['products']))
			{
				foreach ($data['products'] as $product)
				{
					if ($product['quantity'] > 10)
					{
						...
						exit(json_encode(array('code'=>1,'message'=>'订单无法结算')));
					}
					
					$sku_num++;
					$total_num += $product['quantity'];
					$total_money += $product['total'];
				}
			}
			
			if ($total_num> 50 || $sku_num>10 || $total_money > 333)
			{
				$this->db->query('rollback');
				exit(json_encode(array('code'=>1,'message'=>'订单无法结算')));
			}
	...
	public function getOrder($order_id)
	{
		...
		if ($order_query->num_rows)
		{
			...
			return array(
				...
				'shipping_cardno' => $order_query->row['shipping_cardno'],
