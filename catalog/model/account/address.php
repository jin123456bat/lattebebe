...
class ModelAccountAddress extends Model {
	public function addAddress($customer_id, $data) {
		$sql = "INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . "',cardno='".$this->db->escape((string)$data['cardno'])."', fullname = '" . $this->db->escape((string)$data['fullname']) . "', telephone = '" . $this->db->escape((string)$data['telephone']) . "', company = '" . $this->db->escape((string)$data['company']) . "', address_1 = '" . $this->db->escape((string)$data['address_1']) . "', address_2 = '" . $this->db->escape((string)$data['address_2']) . "', postcode = '" . $this->db->escape((string)$data['postcode']) . "', city = '" . $this->db->escape((string)$data['city']) . "',taxno='".$this->db->escape((string)$data['taxno'])."', city_id = '" . $this->db->escape((string)$data['city_id']) . "', county_id = '" . $this->db->escape((string)$data['county_id']) . "', zone_id = '" . (int)$data['zone_id'] . "', country_id = '" . (int)$data['country_id'] . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['address']) ? json_encode($data['custom_field']['address']) : '') . "'";
	...
	{
		$result = $this->db->query('select zipcode from '.DB_PREFIX.'city where city_id='.$id.' limit 1');
		return $result->row['zipcode'];
	}
	...
	public function getAddress($address_id) {
		...
		if ($address_query->num_rows) {
			...
			$address_data = array(
				...
				'taxno' => $address_query->row['taxno'],
				'cardno' => $address_query->row['cardno'],
			);
